...
public:
	...
	std::unique_ptr<ByteBuf> clone(); // Return a new instance of a ByteBuf with the exact same contents and the same state (rpos, wpos)
	//ByteBuf compact(); // TODO?
	bool equals(ByteBuf* other); // Compare if the contents are equivalent
	...
	void put(ByteBuf* src); // Relative write of the entire contents of another ByteBuf (src)
