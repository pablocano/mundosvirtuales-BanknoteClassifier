...
MessageQueue& operator>>(MessageQueue& stream, ColorRGBA&);
MessageQueue& operator<<(MessageQueue& stream, const ColorRGBA&);
...
public:
  ...
  char getDrawingId(const char* name) const
  {
    ...
    if(i != drawings.end())
      return i->second.id;
    OUTPUT(idText, "Warning! Debug drawing " << name << " not declared");
    return -1;
  }
  ...
  const char* getDrawingName(char id) const
  {
    ...
    if(i != drawingsById.end())
      return i->second;
    int idInt = int(id);
    OUTPUT(idText, "Warning! Unknown debug drawing id " << idInt);
    return "unknown";
  }
...
private:
  ...
  const char* getTypeName(char id, char processIdentifier) const
  {
    ...
    if(i != typesById.end())
      return i->second;
    int idInt = int(id);
    OUTPUT(idText, "Warning! Debug drawing has unknown type " << idInt);
    return "unknown";
  }
