...
static RobotCtrl* robot = nullptr;
static bool run = true;

static void bhumanStart()
{
  fprintf(stderr, "BC-Main: Start.\n");
...
{
  {
    ...
    bool watchdog = false;
    //const char* bhDir = "/tmp/BCProcess";
    ...
    int fd = open("/tmp/BCProcess2", O_CREAT, 0600);
    ...
    if(watchdog)
    {
      for(;;)
      {
        ...
        {
          fprintf(stderr, "BC-Main: Error while creating pipes for logging. All logs will be printed on console only! \n");
          pipeReady = false;
        }
