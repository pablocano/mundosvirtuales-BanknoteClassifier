...
void RobotCtrl::connect(SenderList* sender, ReceiverList* receiver)
{
  if(sender && receiver)
    sender->add(receiver);
}

SenderList* RobotCtrl::getSender(const std::string& senderName)
{
  std::string name;
...
ReceiverList* RobotCtrl::getReceiver(const std::string& receiverName)
{
  for(ProcessList::const_iterator i = begin(); i != end(); ++i)
  {
