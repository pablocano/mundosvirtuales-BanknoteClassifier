...
void SPLStandardMessage::toMessageQueue(MessageQueue& in, const unsigned remoteIp)
{
  ...
  if(Global::getSettings()->isDropInGame)
  {
    ...
    in.out.bin << teamNum; // for TeamComm3D
    ...
    if(ballAge >= 0.f)
    {
      ballModel.timeWhenLastSeen = preudoRemoteTimestemp - (unsigned int)(1000*ballAge);
      ballModel.timeWhenDisappeared = preudoRemoteTimestemp - (unsigned int)(1000*ballAge);
      if(ballAge == 0.f)
      {
  ...
  else
  {
    ...
    InBinaryMemory memory(data + uchrtHeaderSize, numOfDataBytes - uchrtHeaderSize);
    memory >> in;
...
unsigned SPLStandardMessage::fromMessageQueue(MessageQueue& out)
{
  ...
  numOfDataBytes = (uint16_t)(sizeStream.getSize() + uchrtHeaderSize);
...
bool SPLStandardMessage::handleMessage(InMessage& message)
{
  ...
  switch(message.getMessageID())
  {
	...
	case idCurrentPositionConfidence:
	{
	  message.bin >> currentPositionConfidence;
	  return true;
	}
	case idCurrentSideConfidence:
	{
	  message.bin >> currentSideConfidence;
	  return true;
	}
	case idAverageWalkSpeed:
	{
		message.bin >> averageWalkSpeed;
		return true;
	}
	case idMaxKickDistance:
	{
		message.bin >> maxKickDistance;
		return true;
	}
