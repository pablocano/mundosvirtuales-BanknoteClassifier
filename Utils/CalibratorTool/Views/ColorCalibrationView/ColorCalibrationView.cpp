...
ColorCalibrationView::ColorCalibrationView(const QString& fullName, Controller& controller)
  : controller(controller), widget(nullptr), fullName(fullName), icon(":/Icons/tag_green.png")
{}

CalibratorTool::Widget* ColorCalibrationView::createWidget()
{
...
{
  SYNC_WITH(colorCalibrationView.controller);
  if(colorCalibrationView.controller.colorTableTimeStamp != timeStamp)
  {
    timeStamp = colorCalibrationView.controller.colorTableTimeStamp;
    updateWidgets(currentColor);
  }
}
...
{
  colorCalibrationView.controller.saveColorCalibration();
}
